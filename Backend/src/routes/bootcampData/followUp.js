...
router.post('/', async (req, res) => {
  try {
    ...
    res.status(400).send({ error: 'Error saving bootcamp student data', details: error });
  }
});
...
router.put('/:id', async (req, res) => {
    try {
      ...
      if (!updatedFollowUp) {
        return res.status(404).send({ error: 'bootcamp Follow-up not found' });
      }
  
      res.status(200).send(updatedFollowUp);
    } catch (error) {
      res.status(400).send({ error: 'Error updating bootcamp follow-up data', details: error });
    }
  });
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    res.status(200).send({ message: 'bootcamp FollowUp Student deleted successfully' });
  } catch (error) {
